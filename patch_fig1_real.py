...
new_func = r"""def fig1_attack_graph():
    ...
    try:
        ...
        # campaign_id in this dataset is what we'll use
        campaign_counts = attack_df['campaign_id'].value_counts()
        valid_campaigns = campaign_counts[(campaign_counts >= 3) & (campaign_counts <= 6)].index.tolist()
...
# Replace the old fig1 function
pattern = re.compile(r"def fig1_attack_graph\(df\):.*?return out", re.DOTALL)
code = pattern.sub(new_func, code)

# Update the main block to call fig1 without arguments
main_pattern = re.compile(r"paths\.append\(fig1_attack_graph\(df\)\)")
code = main_pattern.sub(r"paths.append(fig1_attack_graph())", code)
