...
new_func = r"""def fig1_attack_graph(df):
    ...
    for c_idx, cluster_id in enumerate(campaigns_to_show):
        ...
        if len(chain_nodes) > 0:
            ax.add_patch(plt.Rectangle((-0.8, y_pos - 0.75), 
                                     (len(chain_nodes)-1)*3.5 + 1.6, 1.6, 
                                     fill=True, color="#f8fafc", alpha=0.6, 
                                     edgecolor="#cbd5e1", lw=1, ls="--", zorder=0))
            ax.text(-1.0, y_pos, f"Campaign {cluster_id}", 
    ...
    for n in G.nodes():
        ...
        # Top label (IPs)
        ip_label = f"Src: {src}\nDst: {dst}"
        ax.text(pos[n][0], pos[n][1] + 0.25, ip_label, ha="center", va="bottom", fontsize=9, 
        ...
        ax.text(pos[n][0], pos[n][1] - 0.22, bottom_label, ha="center", va="top", fontsize=10, 
